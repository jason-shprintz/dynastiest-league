import { BlogPost } from "../../types";

const blogPosts: BlogPost[] = [
  {
    id: "welcome-2025-12-29",
    title: "Welcome to the Dynastiest League Blog",
    date: "2025-12-29",
    content:
      "Welcome to the official blog of the Dynastiest League! This is where I'll share league updates, highlight memorable moments, and celebrate our community. Stay tuned for more posts as we continue to build our dynasty legacy together.",
  },
  {
    id: "draft-lottery-recap-2026-01-03",
    title: "Draft Lottery Entries and Draft Order",
    date: "2026-01-03",
    content:
      "The last four picks have already been determined based on the order of elimination from the playoffs.\n\n10: jeffgottfried\n9: JuiceGonLoose\n8: UzUrHead\n7: ThatDudeStu\n\nThe remaining six picks will be decided by a draft lottery, with entries weighted according to each team's regular season record and placing in the loser bracket. Here are the entries for the lottery:\n\n- swiff22: 1 entry\n- Casino1000rr: 1 entry\n- awco: 2 entries\n- dendenoodles: 3 entries\n- mrnickelz12: 3 entries\n- RadLantern: 4 entries\n\nThe draft lottery will take place on <TextHighlight>Sunday January 11th during HALFTIME of the SECOND Wildcard game.</TextHighlight> The full draft order will be announced shortly thereafter along with the annual State of the League and any impending rule changes. Good luck to all teams!",
  },
  {
    id: "draft-lottery-results-2026-01-11",
    title: "Draft Lottery Results and Final Draft Order",
    date: "2026-01-11",
    content:
      "The draft lottery took place during halftime of the second wildcard game on January 11th. Here are the results of the lottery and the full draft order:\n\n1: Awco\n2: Casino1000rr\n3: RadLantern\n4: mrnickelz12\n5: dendenoodles\n6: swiff22\n7: ThatDudeStu\n8: UzUrHead\n9: JuiceGonLoose\n10: jeffgottfried\n\nCongratulations to Awco for securing the first overall pick (But really congratulations to RadLantern who has Awco's pick 😂)! Stay tuned for the annual State of the League and any upcoming rule changes.",
  },
  {
    id: "state-of-the-league-2026",
    title: "State of the League 2026",
    date: "2026-01-25",
    content: `After making some tremendous moves, Jeff, aka Jaxon Flacon Waxon, turned around his team from the loser that it was to the championship team it is today. He came in first place in the regular season, both in points and in record, and triumphed in the playoffs beating out former champion Chris, aka World Breaker, for the title. Once again, the Dynastiest League has a new champion, and once again that champion is not me. Not that I’m bitter or anything.
      
    Congratulations to Jeff for a fantastic season. As always, next year will commence immediately upon the conclusion of the Super Bowl.
    
    <TextHighlight>Rule Changes for 2026:</TextHighlight>
    
    1. There will be Unsportsmanlike Conduct penalties that will be put in place moving forward. A full description of the Unsportsmanlike Conduct penalties is available in the <NavLink section="constitution" subsection="unsportsmanlike-conduct">Constitution section</NavLink>. It is every managers responsibility to understand what these rules are and how to abide by them.
2. We will be instituting a loser's punishment moving forward for the team that finishes last in the losers' bracket. Moving forward, the champion will get to choose the team's name and avatar of the loser. Creativity is encouraged, as is embarrassment, provided it does not breach into the realm of Unsportsmanlike. Please refer to the <NavLink section="constitution" subsection="losers-punishment">Constitution section</NavLink> for additional information.
    
    <TextHighlight>Announcements:</TextHighlight>
    
    1. There was a vote to push the playoffs back one week starting in Week 16 instead of Week 15. That vote failed, and the playoffs will continue to start in Week 15.
    2. After reviewing the lottery process it is concluded that it is as fair as it can be. I will continue to review it for further improvements, and if any managers have suggestions, they are encouraged to share them, but there will be no changes regarding the draft lottery moving into this next year.
    3. I am working to integrate more features in to the website that will make our league better than it has ever been. As always, suggestions are welcome.
    
    <TextHighlight>Draft Details:</TextHighlight>
    
    The draft will be the same as it is every year, a slow draft, meaning everyone will have 24 hours to make a pick. The NFL Draft is April 23-25, so our draft will be two weeks after. The official date for our draft is <TextHighlight>Thursday May 7th, 2026 at 1pm EST, 10am PST.</TextHighlight>`,
  },
];

export { blogPosts };
