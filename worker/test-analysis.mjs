...
const mockRosters = [
  {
    ...
    owner_id: 'user1',
    players: [],
    settings: { wins: 8, losses: 5, fpts: 1450 },
  },
  {
    roster_id: 3,
    owner_id: 'user3',
    players: [],
    settings: { wins: 4, losses: 9, fpts: 1100 },
  },
];
...
const mockUsers = [
  {
    ...
    username: 'jshprintz',
    display_name: 'Jason',
    metadata: { team_name: "Olde' Bowl" },
  },
  {
    user_id: 'user3',
    username: 'player3',
    display_name: 'Mike',
    metadata: { team_name: 'Fish and Gritz' },
  },
];
...
function buildContext(trade, rosters, users, playerNames) {
  ...
  const resolvePlayer = (pid) => {
    if (playerNames[pid])
      return `${playerNames[pid].name} (${playerNames[pid].position})`;
  ...
  for (const rosterId of trade.roster_ids) {
    ...
    gotPicks.forEach(
      (p) =>
        (ctx += `    - ${p.season} Round ${p.round} Pick (originally ${getName(p.roster_id)}'s)\n`),
    );

    ctx += `  Gave up:\n`;
    const sentPlayers = Object.entries(adds).filter(
      ([, to]) => to !== rosterId && trade.roster_ids.includes(to),
    );
    ...
    sentPicks.forEach(
      (p) => (ctx += `    - ${p.season} Round ${p.round} Pick\n`),
    );
  }
  return ctx;
}
...
async function run() {
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Analyze this dynasty fantasy football trade:\n\n${context}\n\nKey the "teams" object by roster ID string (e.g. "1", "3").`,
      },
    ],
  });
